...
typedef struct  
{
    struct list_head list;
    struct list_head node_head;
    uint8 seq; 				//序列号，用来检测丢包/重传/乱序
    uint8 status;  			//帧类型 / 控制标志
    uint16 sort;			//排序号（真正决定播放时间顺序）
    uint32 timestamp;		//时间戳（本地计时）
	uint32 identify_num;	//对讲会话识别号
	uint32 node_cnt;		//这一帧用了多少个 audio_node节点存数据
} __attribute__((packed)) sublist;
